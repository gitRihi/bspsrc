...
public class OccluderMapper {
    ...
    private boolean occFacesContainsBrushFace(DOccluderPolyData dOccluderPolyData, DBrush dBrush, DBrushSide dBrushSide) {
        return VectorUtil.matchingAreaPercentage(WindingFactory.fromOccluder(bsp, dOccluderPolyData), WindingFactory.fromSide(bsp, dBrush, dBrushSide)) > 0; // <- May need to be some epsilon instead of 0
    }
